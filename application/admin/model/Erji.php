...
class Erji  extends Model
{
    ...
    //获取栏目列表
    public static function erji($pid=0,$level=-1)
    {
     ...
     *@return array
     */
    public static function erjiacateall($data,$pid=0,$level=-1,&$arr=[])
    {
        // $res=self::where('pid',$pid)->select();
        //static $arr=array();
        $level+=1;//$level=$level+1;
        ...
        foreach($data as $v){
            if($pid==$v['pid']){
                ...
                $tmp_arr['type']=$v['type'];
                $arr[]= $tmp_arr;
                self::erjiacateall($data,$v['id'],$level,$arr);
                unset($v);
            }
